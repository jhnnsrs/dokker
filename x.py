...
class ThisProject(SetupProject):

    async def aget_health_checks(self) -> list:

        list = []
        if not self._setup:
            raise ValueError("Setup not initialized")

        if self._setup.lok.enabled:
            list.append(HealthCheck(
                name="HT",
                service="lok",
                url=f"http://localhost:{self._setup.lok.port}/ht?format=json",
                method="GET",
                max_retries=5,
                timeout=5,
                status_code=200,
...
project = ThisProject(
    template_dir="tests/templates/arkitekt", 
    project_name="arkitekt",
    setup_class=Setup,
)
...
def arkitekt_deployment(project_name: str) -> Deployment[Setup]:
    project = ThisProject(
        template_dir="tests/templates/arkitekt", 
    ...
    return Deployment(project=project)
...
def main():
    ...
    with arkitekt_deployment("arkitekt") as deployment:
        ...
        t = deployment.get_config()
        print(t.admin_password)
        print(t.admin_username)
        ...
        print(deployment.inspect().services.get("lok").ports)
        
        with deployment.watch_logs("lok"):
            deployment.check_health()


        orkestrator_link = f'http://127.0.0.1:{t.orkestrator.port}/?endpoint=localhost:{t.lok.port}'
        print(orkestrator_link)
        webbrowser.open(orkestrator_link)
                        



        with deployment.watch_logs("lok", live=True):
